...
def LSIGF(h, S, x, b=None):
    ...
    if not S.is_sparse:
        ...
        x = x.squeeze().reshape((-1, N)).T
        # x = x.squeeze().permute((3,0,1,2))
        z = x
    ...
    # if S.is_sparse:
    #     x = x.squeeze().reshape((-1, N))

    for k in range(1,K):
        if S.is_sparse:
            x = torch.sparse.mm(S,x) # N x BEG
            z = torch.cat((z,x), dim=0) # Nk x BEG
            # xS = x.T.view((B, 1, 1, G, N))
    ...
    if S.is_sparse:
        z = z.to_dense().T.reshape((B, 1, K, G, N))
    # z = torch.zeros([B, E, K, G, N])
